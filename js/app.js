...
let slider_popular = new Swiper('.popular__body', {
	observer: true,
	observeParents: true,
	slidesPerView: 5,
	spaceBetween: 0,
...
if (iconMenu != null) {
    ...
    if (catalogBody.classList.contains("_active")) {
      iconCatalog.classList.remove("_active");
...
//Search Block
const iconSearch = document.querySelector(".header__search-button");
const searchBody = document.querySelector(".header__search-block");
if (iconSearch != null) {
...
//Catalog
const iconCatalog = document.querySelector(".catalog-button");
const catalogBody = document.querySelector(".catalog-header__menu");
if (iconCatalog != null) {
	iconCatalog.addEventListener("click", function (e) {
    e.stopPropagation();
    iconCatalog.classList.toggle("_active");
    catalogBody.classList.toggle("_active");
	});
};
//Close menu, catalog, search block
document.addEventListener('click', function(e) {
  const target = e.target;
  const its_catalogBody = target == catalogBody || catalogBody.contains(target);
  const its_iconCatalog = target == iconCatalog;
  const its_searchBody = target == searchBody || searchBody.contains(target);
  const its_iconSearch = target == iconSearch;
  const its_menuBody = target == menuBody || menuBody.contains(target);
  const its_iconMenu = target == iconMenu;
  if (!its_catalogBody && !its_iconCatalog && catalogBody.classList.contains('_active')) {
    iconCatalog.classList.remove("_active");
    catalogBody.classList.remove("_active");
  }
  if (!its_searchBody && !its_iconSearch && searchBody.classList.contains('_active')) {
    searchBody.classList.remove("_active");
  }
  if (!its_menuBody && !its_iconMenu && menuBody.classList.contains('_active')) {
    iconMenu.classList.remove("_active");
    menuBody.classList.remove("_active");
  }
});
...
const input = document.body.querySelector('.form__input_tel');
...
//slide image
const slideImages = document.querySelectorAll('.product__slide > img');
let slideImageMain = document.querySelector('.product__image > img');
for (let i = 0; i < slideImages.length; i++) {
	let slideImage = slideImages[i];
	slideImage.addEventListener('click', function(e) {
		e.stopPropagation();
		slideImageMain.src = slideImage.src;
	});
}
// Dynamic Adapt v.1
